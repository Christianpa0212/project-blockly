...
function resetButtonClick(e) {
  ...
  // === HOOK: al reiniciar, crear un nuevo intento en el backend (Bird = juego_id 2)
  try {
    ...
    // Bird usa niveles 11–20, mapeando level=1..10 => nivel_id=11..20
    const levelParam = parseInt(params.get('level'), 10);
    const nivel_id = Number.isFinite(levelParam)
      ? 10 + levelParam      // 1->11, 2->12, ..., 10->20
      : 11;                  // fallback razonable: primer nivel de Bird

    // Si ya había intento previo y hay sesión válida, abrimos un nuevo intento
    if (intentoAnterior && sesion && sesion.sesion_id) {
      console.log('Cerrando intento previo y creando nuevo intento (Bird)...');
      ...
      fetch('/player/try/start', {
        ...
        .then(res => {
          if (res.ok && res.intento_id) {
            ...
            console.log('Nuevo intento Bird iniciado tras reinicio:', res.intento_id);

            // Estado global del intento en front
            window.TRY_STATUS = 'open';
            window.SKIP_ABANDON = false;
          } else {
            console.error('Error al iniciar nuevo intento Bird:', res.error || res);
          }
        })
        .catch(err => console.error('Error en nuevo intento Bird:', err));
    }
  } catch (err) {
    console.error('Error creando nuevo intento Bird tras reinicio:', err);
  }

  // Reset gráfico / de lógica del juego
  reset(false);
}
...
function execute() {
  ...
  //    Infinity is thrown.
  // 3. If another error occurs [ERROR], false or an Error is thrown.
  ...
  try {
    ...
    // Si terminó sin lanzar nada, no llegó al objetivo => FAILURE
  ...
  // === HOOK: si el resultado fue FAILURE / ERROR / TIMEOUT,
  // cerramos el intento como FALLADO (estado_id = 5)
  if (
    result === ResultType.FAILURE ||
    result === ResultType.ERROR ||
    result === ResultType.TIMEOUT
      ...
      if (intento_id) {
        ...
        fetch('/player/try/end', {
          ...
          body: JSON.stringify({
            ...
            es_reinicio: 1        // se supone que luego harán reset
          })
        })
          .then(r => r.json())
          .then(res => console.log('/player/try/end FALLADO (Bird):', res))
          .catch(err => console.error('Error registrando fallo Bird:', err));
      }
    } catch (err) {
      console.error('Error general al registrar fallo Bird:', err);
    }

    // Estado global para el lock de navegación / edición (ANL.js)
...
function animate() {
  ...
  if (action[0] === 'move' || action[0] === 'goto') {
    ...
    // === HOOK: intento COMPLETADO (estado_id = 3) ===
    // - Marcamos TRY_STATUS para que el lock de UI/navegación lo sepa.
    // - SKIP_ABANDON evita que, al salir del nivel tras el éxito, se marque ABANDONO en unload.
    ...
    try {
      ...
      if (raw) {
        const intento_id = parseInt(raw, 10);

        // Número de bloques actuales en el workspace
        const bloques = (window.Blockly && Blockly.mainWorkspace)
          ? Blockly.mainWorkspace.getAllBlocks().length
          : 0;

        // Código JS plano (sin block_id_*, por líneas)
        let codigoPlano = [];
        try {
          const jsCode = (BlocklyCode && BlocklyCode.executedJsCode)
        ...
        // Leer sesión para mandar Authorization
        let sesion = null;
        try {
          sesion = JSON.parse(localStorage.getItem('blocklygames') || 'null');
        } catch {
        ...
        fetch('/player/try/end', {
          ...
          body: JSON.stringify({
            ...
            num_bloques: bloques,
            codigo_js: codigoPlano,
            es_reinicio: 0       // éxito normal, no es reinicio
          })
        }).catch(() => {});
      }
    } catch (_) {
      // Si algo truena aquí, no rompemos la animación
    }

    // Flujo original de Bird: guardar y mostrar popup de felicitaciones
