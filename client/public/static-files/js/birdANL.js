/* ============================================================
   Abandono + navegación + lock de edición (Bird)
...
(function () {
  ...
  function isWhitelistedTarget(t) {
    ...
    // Permitimos clicks dentro del diálogo de Blockly
  ...
  function ensureGlobalGuards(bind) {
    if (bind) {
      ...
      if (!__guardsBound) return;
      __guardsBound.forEach?.(() => {}); // safeguard
  ...
  // ----------------------------------------------------------
  // Back/Forward del navegador: bloqueados (clave distinta para Bird)
  // ----------------------------------------------------------
  try { history.pushState({ holdBird: true }, '', location.href); } catch(_) {}
  ...
  // ----------------------------------------------------------
  // Detectar ÉXITO o FALLA (mismos hints genéricos)
  // ----------------------------------------------------------
  const successHints = [
